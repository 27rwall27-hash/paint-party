 ...
 * "write straight onto the live object, no confirm step" pattern as Stampede Sprint's setupUI.ts.
 * Picking a color already assigned to a CPU swaps it away instead of leaving two players visually
 * identical — with exactly PLAYER_COUNT colors for PLAYER_COUNT identities, every color is always
 * somebody's default, so a plain "pick" would otherwise always collide with exactly one CPU. */
